<template>
  <div class="mx-auto flex flex-col items-center justify-center min-h-screen p-6">
    <input type="file" @change="onFileChange" class="mb-4 p-2 border border-gray-300 rounded-md shadow-2xs" />
    <div v-if="imageUrl" class="w-full max-w-md">
      <img ref="image" :src="imageUrl" alt="Source Image" class="w-full h-auto mt-4 border border-gray-300 rounded-md shadow-2xs" />
    </div>
    ...
    <div v-if="croppedImageUrl" class="w-full max-w-md mt-8">
      ...
      <img :src="croppedImageUrl" alt="Cropped Image" class="w-full h-auto border border-gray-300 rounded-md shadow-2xs" />
    </div>
  </div>
</template>
