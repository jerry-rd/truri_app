...
<script setup>
  ...
  let dye, velocity, divergence, curl, pressure, gl, blit, canvas, config, ext, pointers
  ...
  let baseVertexShader, copyShader, clearShader, splatShader, advectionShader, divergenceShader, curlShader, vorticityShader, pressureShader, gradientSubtractShader
  ...
  class Material {
    ...
    bind() {
      if (!this.activeProgram) {
        this.setKeywords([])
        if (!this.activeProgram) {
          console.warn('Failed to initialize program in Material')
          return false
        }
      }
  ...
  class Program {
    ...
    bind() {
      if (!this.program) {
        console.warn('No program available')
        return false
      }
  ...
  // 保留所有原React组件中的函数和类定义
  // 这里需要将原组件中的所有函数、类定义复制过来
  // 包括：pointerPrototype, getWebGLContext, Material, Program 等所有辅助函数
  // 由于代码量较大，这里只展示框架结构，具体实现需完整复制

  onMounted(() => {
    canvas = canvasRef.value
    if (!canvas) return

    const { gl: gls, ext: exts } = getWebGLContext(canvas)
    gl = gls
    ext = exts
    ...
    blit = (() => {
      ...
      return (target, clear = false) => {
        if (!gl.getParameter(gl.CURRENT_PROGRAM)) {
          console.warn('No shader program bound before blit!')
          return
        }
    ...
    console.log('🚀 ~ onMounted ~ pointers:', pointers)
    ...
    if (!displayMaterial || !displayMaterial.activeProgram) {
      console.warn('Display material not properly initialized')
      displayMaterial = new Material(baseVertexShader, displayShaderSource)
      updateKeywords()
    }
  ...
  function initFramebuffers() {
    ...
    if (!copyProgram || !clearProgram || !splatProgram || !advectionProgram || !divergenceProgram || !curlProgram || !vorticityProgram || !pressureProgram || !gradienSubtractProgram || !displayMaterial) {
      console.warn('Some shader programs not initialized')
      initShaders()
    }
  }
  ...
  function updateKeywords() {
    if (!displayMaterial) {
      console.warn('Display material not available for keyword update')
      return
    }
    ...
    try {
      ...
      console.error('Error updating display material keywords:', e)
    }
  }
  ...
  function render(target) {
    if (!displayMaterial) {
      console.warn('Display material not initialized')
      return
    }
    ...
    if (!displayMaterial.activeProgram) {
      updateKeywords()
      if (!displayMaterial.activeProgram) {
        console.warn('Failed to initialize display material program')
        return
      }
    }
    ...
    if (!displayMaterial.bind()) {
      console.warn('Failed to bind display material')
      return
    }
