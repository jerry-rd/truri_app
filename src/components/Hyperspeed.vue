...
<script setup>
  ...
  const props = defineProps({
    effectOptions: { type: Object, required: true },
  })

  const hyperspeedRef = ref(null)

  // 默认配置选项
  let defaultEffectOptions = {
    onSpeedUp: () => {},
    onSlowDown: () => {},
  ...
  watch(
    () => props.effectOptions,
    (val) => {
      defaultEffectOptions = { ...defaultEffectOptions, ...val }
      defaultEffectOptions.distortion = distortions[defaultEffectOptions.distortion]
      const myApp = new App(hyperspeedRef.value, defaultEffectOptions)
