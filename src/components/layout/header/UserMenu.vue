<template>
  <div class="relative" ref="dropdownRef">
    <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown">
      ...
      <span class="block mr-1 font-medium text-theme-sm">张三 </span>
    ...
    <div v-if="dropdownOpen" class="absolute right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark">
      <div>
        <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400"> 张三 </span>
        <span class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400"> 1234567890@qq.com </span>
      </div>
      ...
      <router-link to="/admin/login/signin" @click="signOut" class="flex items-center gap-3 px-3 py-2 mt-3 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
...
<script setup>
  ...
  const menuItems = [
    { href: '/admin/main/UserProfile', icon: UserCircleIcon, text: '编辑个人资料' },
    { href: '/chat', icon: SettingsIcon, text: '账户设置' },
    { href: '/profile', icon: InfoCircleIcon, text: '支持' },
  ]
